...
/**
 * Tests for the GROUP-account email route
 * (`GET`/`POST`/`PUT` /api/groups/[groupDid]/account/email).
 *
 * `callPds` is mocked to return either `locked` or a stubbed PDS `response`,
 * so we assert: locked → `401 { error: "locked" }`; success shapes; upstream
 * error forwarding; and the auth / DID-validity / rate-limit / CSRF /
 * validation gates. The `callPds` helper itself is unit-tested separately.
 */
...
function makeRequest(method: string, body?: unknown): Request {
  return new Request(
    `https://example.test/api/groups/${GROUP_DID}/account/email`,
    {
      method,
      headers: {
...
async function get(groupDid = GROUP_DID): Promise<Response> {
  const { GET } = await import("../route")
  return GET(
    makeRequest("GET") as unknown as Parameters<typeof GET>[0],
    makeContext(groupDid) as unknown as Parameters<typeof GET>[1],
  )
}

async function post(groupDid = GROUP_DID): Promise<Response> {
  const { POST } = await import("../route")
  return POST(
    makeRequest("POST") as unknown as Parameters<typeof POST>[0],
...
describe("POST (request email update)", () => {
  ...
  it("400s when the group DID is invalid", async () => {
    const res = await post("not-a-did")
    expect(res.status).toBe(400)
    expect(callPds).not.toHaveBeenCalled()
  })
...
describe("PUT (update email)", () => {
  ...
  it("400s when the group DID is invalid", async () => {
    const res = await put({ email: "new@example.com" }, "not-a-did")
    expect(res.status).toBe(400)
    expect(callPds).not.toHaveBeenCalled()
  })
  ...
  it("forwards the upstream status + message when updateEmail fails", async () => {
    vi.mocked(callPds).mockResolvedValueOnce({
      kind: "ok",
      response: jsonResponse({ error: "TokenInvalid", message: "bad token" }, 400),
    })
