 ...
 *
 * Module layout: this file holds the shared plumbing (proxy URL,
 * {@link postIndexer}) and the activity-connection core; the other
 * indexer domains live in sibling modules — `indexer-funding`,
 * `indexer-closure`, `indexer-counts`, `indexer-collections` — and
 * are re-exported at the bottom so every existing
 * `@/lib/atproto/indexer` import keeps resolving unchanged.
 */
 ...
 * by-URIs batch fetchers here and in `indexer-collections` (the proxy
 * caps `uri: { in: [...] }` lists at 50 values per request).
 */
...
// The sibling modules import only the shared plumbing above (postIndexer,
// INDEXER_PROXY_URL, chunkArray), never each other, so the circular edge
// created by these re-exports is init-order-safe: no module dereferences
// a cross-module binding until call time.
// ---------------------------------------------------------------------------
