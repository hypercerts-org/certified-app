 ...
 *
 * The glyphs animate their own meaning, scrubbed by scroll
 * (animation-timeline: view(), see landing.css): the profile's dark
 * arc completes its circle, the record's highlight climbs the stack
 * row by row, and the supporters' connections and circles light up in
 * turn. Element opacities in the markup are the finished states —
 * browsers without scroll timelines and reduced-motion users render
 * those directly.
 */
...
const ITEMS = [
  {
    ...
    glyph: (
      // One identity: the signature arc completes the ring as you scroll.
  ...
  },
  {
    ...
    glyph: (
      // A record accumulating: the highlighted entry climbs the stack.
      <svg viewBox="0 0 64 64" fill="none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
        <rect x="12" y="38" width="40" height="12" rx="1" stroke="currentColor" strokeWidth="0.9" opacity="0.22" className="lp-glyph-rec" style={{ animationName: "lpRecA" }} />
        <rect x="12" y="31" width="40" height="12" rx="1" stroke="currentColor" strokeWidth="0.9" opacity="0.22" className="lp-glyph-rec" style={{ animationName: "lpRecB" }} />
        <rect x="12" y="24" width="40" height="12" rx="1" stroke="currentColor" strokeWidth="0.9" opacity="0.22" className="lp-glyph-rec" style={{ animationName: "lpRecC" }} />
        <rect x="12" y="17" width="40" height="12" rx="1" stroke="currentColor" strokeWidth="0.9" opacity="0.9" className="lp-glyph-rec" style={{ animationName: "lpRecD" }} />
      </svg>
    ),
  },
  {
