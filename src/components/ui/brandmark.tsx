import React from "react";

type BrandmarkProps = React.SVGProps<SVGSVGElement> & {
  size?: number | string;
  title?: string;
  /**
   * Mark the brandmark as purely decorative. When the brandmark sits inside
   * an element that is already labelled (e.g. a link whose text or aria-label
   * names the destination), exposing the brandmark's own label produces a
   * redundant announcement. Set `decorative` to hide it from the a11y tree
   * (`aria-hidden`, no `role="img"`, no `<title>`/`aria-label`). Defaults to
   * `false`, preserving the labelled behaviour.
   */
  decorative?: boolean;
};

/**
 * Certified brandmark — inline SVG.
 *
 * The rounded square inherits its color from the parent element's `color`
 * token (drive it with `color: var(--fg-primary)` for the standard
 * theme-aware look). The "C" mark inside is filled with `--bg-canvas` so
 * it inverts against the background in both light and dark themes.
 *
 * Source: github.com/hypercerts-org/certified-app, public/brand/brandmark/
 */
export default function Brandmark({
  size = 40,
  title = "Certified",
  decorative = false,
  className,
  ...rest
}: BrandmarkProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      role={decorative ? undefined : "img"}
      aria-label={decorative ? undefined : title}
      aria-hidden={decorative ? true : undefined}
      className={className}
      {...rest}
    >
      {!decorative && <title>{title}</title>}
      <path
        d="M0 8C0 3.58172 3.58172 0 8 0H32C36.4183 0 40 3.58172 40 8V32C40 36.4183 36.4183 40 32 40H8C3.58172 40 0 36.4183 0 32V8Z"
        fill="currentColor"
      />
      <path
        d="M29.8965 22.5791C29.7197 23.9952 29.2851 25.2627 28.5918 26.3828C27.8974 27.5029 26.9757 28.3863 25.8262 29.0312C24.6754 29.6762 23.3451 29.9999 21.835 30C20.2149 30 18.7905 29.6896 17.5645 29.0693L19.5527 27.167C20.2356 27.404 20.995 27.5273 21.835 27.5273C23.3104 27.5273 24.5302 27.069 25.4961 26.1553C26.4621 25.2414 27.0424 24.0493 27.2354 22.5801V22.5791H29.8965ZM21.6172 22.0518L14.4609 28.8994C13.792 29.5396 12.7408 29.5057 12.1133 28.8232L11.6914 28.3643L9 25.4336L10.8262 23.6875L13.3633 26.4492L19.9062 20.1895L21.6172 22.0518ZM21.833 10C23.2554 10 24.529 10.2916 25.6533 10.874C26.7777 11.4565 27.6864 12.2634 28.3809 13.2939C29.074 14.3244 29.5349 15.4942 29.7637 16.8018H27.1025C26.8564 15.5118 26.2602 14.467 25.3115 13.6699C24.3628 12.8716 23.2381 12.4727 21.833 12.4727C20.4281 12.4727 19.2423 12.8052 18.2764 13.4678C17.3104 14.1304 16.5955 15.0321 16.1299 16.1699C15.6643 17.3089 15.4316 18.5861 15.4316 20.001C15.4317 20.4418 15.4577 20.8677 15.5039 21.2803L15.501 21.2812L13.1689 23.5137C12.8524 22.4183 12.6905 21.2482 12.6904 20.002C12.6904 18.1566 13.0379 16.4762 13.7324 14.9609C14.4257 13.4457 15.4623 12.2409 16.8408 11.3447C18.2192 10.4487 19.8839 10.0001 21.833 10Z"
        fill="var(--bg-canvas)"
      />
    </svg>
  );
}
